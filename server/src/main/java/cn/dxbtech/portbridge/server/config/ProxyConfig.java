package cn.dxbtech.portbridge.server.config;
...
import cn.dxbtech.portbridge.server.ProxyChannelManager;
...
public class ProxyConfig implements Serializable {
     ...
     * 配置文件为mapping.json
     */
    public static final String CONFIG_FILE = PersistenceUtil.getFilePath("mapping.json");
    ...
    private ProxyConfig() {
        ...
        logger.info("build on {}:{}, config: {}:{}@{}:{}",
                serverBind, serverPort, configAdminUsername, configAdminPassword, configServerBind, configServerPort);
        update(null);
    }
    ...
    public List<Client> getClients(boolean updateStatus) {
        if (updateStatus) {
    ...
    public void update(String proxyMappingConfigJson) {
        ...
        Path file = Paths.get(ProxyConfig.CONFIG_FILE);
        try {
            if (proxyMappingConfigJson == null && Files.exists(file)) {
                proxyMappingConfigJson = new String(Files.readAllBytes(file), "UTF-8");
            }
        } catch (Exception e) {
        ...
        if (proxyMappingConfigJson != null) {
            try {
                Files.write(file, proxyMappingConfigJson.getBytes(Charset.forName("UTF-8")));
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        }
    ...
    public static class Client implements Serializable {
        ...
        public Client(String name, String clientKey, List<ClientProxyMapping> proxyMappings) {
            this.name = name;
    ...
    public static class ClientProxyMapping {
        ...
        public ClientProxyMapping(String name, Integer inetPort, String lan) {
