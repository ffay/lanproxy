package cn.dxbtech.portbridge.client.handlers;

import cn.dxbtech.portbridge.client.ClientChannelMannager;
import cn.dxbtech.portbridge.client.listener.ChannelStatusListener;
import cn.dxbtech.portbridge.client.listener.ProxyChannelBorrowListener;
import cn.dxbtech.portbridge.commons.Config;
import cn.dxbtech.portbridge.protocol.Constants;
...
import static cn.dxbtech.portbridge.protocol.ProxyMessage.getTypeName;
...
public class ClientChannelHandler extends SimpleChannelInboundHandler<ProxyMessage> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, ProxyMessage proxyMessage) {
        if (logger.isDebugEnabled()) {
            logger.debug("recieved proxy message, type is {}", getTypeName(proxyMessage.getType()));
        }
